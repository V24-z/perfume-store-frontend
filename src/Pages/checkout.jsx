...
/* ---------------------------- Inline icons ---------------------------- */
/* Kept as plain inline SVG so no extra icon dependency is introduced. */
    ...
    <div className="flex min-h-[70vh] items-center justify-center bg-[#FAF9F6] px-6 py-20">
      ...
      >
        ...
        <p className="mx-auto mt-2 max-w-sm text-[#6B7280]">
          ...
          you&apos;ll love and come back to check out.
        </p>
        <motion.button
          type="button"
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onContinue}
          className="mt-8 inline-flex items-center justify-center rounded-xl bg-[#111827] px-8 py-3.5 font-semibold text-white shadow-lg shadow-black/10 transition-colors hover:bg-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#111827] focus-visible:ring-offset-2"
        >
          Continue shopping
        </motion.button>
      </motion.div>
...
function Checkout() {
  ...
  // Derived, display-only values. cartItems / formData / loading remain the
  // only pieces of real state — nothing here feeds back into business logic.
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] py-8 sm:py-12">
      ...
      >
        {/* Header */}
        <div className="mb-8">
          <p className="mb-1 text-sm font-semibold text-[#0B6E4F]">
            Step 2 of 2
          </p>
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[1fr_400px] lg:gap-10">
          ...
          >
            ...
            <motion.section
              ...
              className="rounded-2xl bg-white p-6 ring-1 ring-black/5 shadow-sm shadow-black/5 sm:p-8"
            >
              <h2
                id="shipping-heading"
                className="mb-6 flex items-center gap-2.5 text-lg font-semibold text-[#111827]"
              >
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#F0ECE3] text-xs font-bold text-[#0B6E4F]">
                  1
                </span>
                Shipping information
              </h2>
              ...
              >
                <motion.div variants={fieldVariant}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-[#E7E2D9] bg-[#FCFBF9] px-4 py-3 text-[#111827] placeholder:text-[#9CA3AF] transition-colors duration-200 focus:border-[#0B6E4F] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0B6E4F]/20"
                  />
                </motion.div>
                ...
                <motion.div variants={fieldVariant}>
                  <div className="relative">
                    ...
                    <span className="absolute left-12 top-[80%] -translate-y-1/2 text-slate-300 pointer-events-none">
                      |
                    </span>
                    <input
                      id="phone"
                      ...
                      onChange={handleChange}
                      className="w-full rounded-xl border border-[#E7E2D9] bg-[#FCFBF9] px-4 py-3 pl-16  text-[#111827] placeholder:text-[#9CA3AF] transition-colors duration-200 focus:border-[#0B6E4F] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0B6E4F]/20"
                    />
                  </div>
                </motion.div>
                ...
                <motion.div variants={fieldVariant}>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={4}
                    className="w-full resize-none rounded-xl border border-[#E7E2D9] bg-[#FCFBF9] px-4 py-3 text-[#111827] placeholder:text-[#9CA3AF] transition-colors duration-200 focus:border-[#0B6E4F] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0B6E4F]/20"
                  />
                </motion.div>
              </motion.div>
            </motion.section>
            ...
            <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-black/5 shadow-md shadow-black/5">
              <div className="p-6 sm:p-7">
                ...
                <motion.ul
                  ...
                  className="-mr-1 max-h-80 space-y-4 overflow-y-auto pr-1"
                >
                  {cartItems.map((item) => {
                    const price = itemPrice(item);
                    const image =
                      item.products?.image_url || item.products?.image;
                    return (
                      <motion.li
                        key={item.id}
                ...
                <div className="mt-4 flex items-center justify-between border-t border-[#E7E2D9] pt-4">
                  <span className="font-sans text-base font-semibold text-[#111827]">
                    Total
                  </span>
              ...
              <div className="bg-[#F0ECE3] px-6 py-5 sm:px-7">
                ...
                <motion.button
                  type="button"
                  whileHover={
                    !loading && cartItems.length > 0 ? { scale: 1.015 } : {}
                  }
                  whileTap={
                    !loading && cartItems.length > 0 ? { scale: 0.985 } : {}
                  }
                  onClick={handleCheckout}
                  disabled={loading || cartItems.length === 0}
                  aria-busy={loading}
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#111827] px-6 py-3.5 text-base font-semibold text-white shadow-lg shadow-black/10 transition-colors duration-200 hover:bg-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#111827] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-[#111827]"
                >
